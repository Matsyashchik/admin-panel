...
function* fetchPostsSaga(action: PayloadAction<number>): Generator {
...
function* addPostSaga(action: ReturnType<typeof addPostRequest>): Generator {
  const currentPage: any = yield select((state) => state.posts.currentPage)
...
function* editPostSaga(action: PayloadAction<{ id: number; data: FormData }>): Generator {
  const currentPage: any = yield select((state) => state.posts.currentPage)
...
function* fetchPostDetailSaga(action: ReturnType<typeof fetchPostDetailRequest>): Generator {
  yield handleApiRequest(
...
function* deletePostSaga(action: PayloadAction<number>): Generator {
  const currentPage: any = yield select((state) => state.posts.currentPage)
