...
function* loginSaga(action: ReturnType<typeof login>): Generator {
  try {
    const {email, password} = action.payload
    const response: any = yield call(loginRequest, email, password)
...
function* refreshTokenSaga(): Generator {
  try {
    ...
    const response: any = yield call(refreshTokenRequest, oldRefreshToken)
