...
const schema = yup.object().shape({
  ...
  tagIds: yup.array().min(1, 'Выберите хотя бы один тэг').required(),
    ...
    if (visible && initialValues) {
      ...
      setFileList(
        initialValues?.previewPicture
          ? [{
            uid: initialValues.previewPicture.id.toString(),
  ...
  return (
    ...
    >
      <Form layout="vertical">
        ...
        <Form.Item label="Тэги" validateStatus={errors.tagIds ? 'error' : ''} help={errors.tagIds?.message}>
          <Controller
            name="tagIds"
            control={control}
            render={({field}) => (
